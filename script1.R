...
# The code provides analysis of the Colorectal cancer Dataset (a) as an example
...
# Load the dataset
Colorectal.sparse.m <- Read10X_h5(filename = 'D:/My PhD information folder first Paper/scRNA seq data(cancer)/Filtered Data/Colorectal cancer/Data1/CytAssist_11mm_FFPE_Human_Colorectal_Cancer_filtered_feature_bc_matrix.h5')
str(Colorectal.sparse.m)
cnts <-Colorectal.sparse.m
# Initialize the Seurat object with the filtered/Raw data collected from the 10x genomics database
Colorectalcancer.seurat.obj<- CreateSeuratObject(counts = cnts, project = "Colorectalcancer",min.cells = 3, min.features = 100)
str(Colorectalcancer.seurat.obj)
Colorectalcancer.seurat.obj

#Step 2: Quality control and filtering

View(Colorectalcancer.seurat.obj@meta.data)
# Calculation of mitochondrial genes percentage in each cell
Colorectalcancer.seurat.obj[["percent.mt"]] <- PercentageFeatureSet(Colorectalcancer.seurat.obj, pattern = "^MT-")
View(Colorectalcancer.seurat.obj@meta.data)
#Violin plot showing the distribution of the data
dev.new(height=7, width=7)
VlnPlot(Colorectalcancer.seurat.obj, features = c("nFeature_RNA", "nCount_RNA", "percent.mt"), ncol = 3)
#Scatter plot illustrating the data distribution
dev.new(height=7, width=7)
FeatureScatter(Colorectalcancer.seurat.obj, feature1 = "nCount_RNA", feature2 = "nFeature_RNA") +
  geom_smooth(method = 'lm')
#Filtration to subset the dataset by setting the thresholds(if required) from violin and scatter plots observation(See Table S1)
Colorectalcancer.seurat.obj <- subset(Colorectalcancer.seurat.obj, subset = nFeature_RNA > 1000 & nFeature_RNA < 9500 & 
                                        percent.mt < 10)

#Step 3: Data normalization

Colorectalcancer.seurat.obj<- NormalizeData(Colorectalcancer.seurat.obj,normalization.method = "LogNormalize", scale.factor = 10000)
str(Colorectalcancer.seurat.obj)

#Step4: Top 2000 high variable features identification

Colorectalcancer.seurat.obj <- FindVariableFeatures(Colorectalcancer.seurat.obj, selection.method = "vst", nfeatures = 2000)
# Identifying the 3 most highly variable genes
top3 <- head(VariableFeatures(Colorectalcancer.seurat.obj), 3)
# Plot variable features with and without labels
dev.new(height=7, width=7)
plot1 <- VariableFeaturePlot(Colorectalcancer.seurat.obj)
LabelPoints(plot = plot1, points = top3)

#Step5: Scaling data

all.genes <- rownames(Colorectalcancer.seurat.obj)
Colorectalcancer.seurat.obj <- ScaleData(Colorectalcancer.seurat.obj, features = all.genes)
str(Colorectalcancer.seurat.obj)

#Step6: Perform linear dimensionality reduction technique PCA to identify principal components

Colorectalcancer.seurat.obj<- RunPCA(Colorectalcancer.seurat.obj, features = VariableFeatures(object = Colorectalcancer.seurat.obj))
# Determine the ideal amount of principal components to keep for subsequent analyses by observing the elbow plot 
dev.new(height=7, width=7)
ElbowPlot(Colorectalcancer.seurat.obj)

#Step7: Clustering(See Table S2)

Colorectalcancer.seurat.obj <- FindNeighbors(Colorectalcancer.seurat.obj, dims = 1:12)

#Fitting a resolution to form 8 clusters in the dataset
Colorectalcancer.seurat.obj <- FindClusters(Colorectalcancer.seurat.obj, resolution = c(0.35))
View(Colorectalcancer.seurat.obj@meta.data)
#Dimensional plot showing 8 identified clusters in the dataset derived from PCA
dev.new(height=7, width=7)
DimPlot(Colorectalcancer.seurat.obj, group.by = "RNA_snn_res.0.35", label = TRUE)
# setting identity of the clusters
Idents(Colorectalcancer.seurat.obj)
Idents(Colorectalcancer.seurat.obj) <- "RNA_snn_res.0.35"
Idents(Colorectalcancer.seurat.obj)

#Step8: Perform non-linear dimensionality reduction technique UMAP

Colorectalcancer.seurat.obj <- RunUMAP(Colorectalcancer.seurat.obj, dims = 1:12)
#UMAP dimensional plot illustrating 8 identified clusters
dev.new(height=7, width=7)
DimPlot(Colorectalcancer.seurat.obj, reduction = "umap",label = TRUE)
...
# Get UMAP embeddings
umap_emb <- as.data.frame(Colorectalcancer.seurat.obj@reductions$umap@cell.embeddings)
# Get cluster information
cluster_info <- Colorectalcancer.seurat.obj$RNA_snn_res.0.35
...
for (cluster_id in 0:7) {
  ...
  markers_cluster <- FindConservedMarkers(Colorectalcancer.seurat.obj,
...
# Create an empty list to store metadata for each cluster
  ...
  for (i in 0:7) {
    ...
    cluster_meta <- Colorectalcancer.seurat.obj@meta.data[Colorectalcancer.seurat.obj@meta.data$seurat_clusters == i, ]
  ...
  cell_ids <- rownames(Colorectalcancer.seurat.obj@meta.data)
  # Add the cell IDs to the Seurat object's metadata data.frame
  Colorectalcancer.seurat.obj@meta.data$cell_id <- cell_ids
  ...
  for (i in 0:7) {
    ...
    seurat_subset <- Colorectalcancer.seurat.obj[, Colorectalcancer.seurat.obj@meta.data$cell_id %in% cell_names]
...
write.csv(norm_dataset, file = "Colorectal_1_NormData.csv", row.names = FALSE)

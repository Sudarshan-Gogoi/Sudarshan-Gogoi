...
analyze10xgenomCancer <- function(
  ...
  cluster_ids <- levels(Idents(Cancer.seurat.obj))
  
  # Loop over each cluster
  for (cluster_id in cluster_ids) {
    # Subset the cells belonging to the current cluster
    cluster_cells <- WhichCells(Cancer.seurat.obj, ident = cluster_id)
    
    # Get the gene expression matrix for the current cluster
    cluster_expr_matrix <- GetAssayData(Cancer.seurat.obj, cells = cluster_cells, assay = "RNA")
  ...
  # Hausdorff distance matrix construction

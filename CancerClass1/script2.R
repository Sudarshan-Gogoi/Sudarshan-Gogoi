...
for (i in 1:ncol(column_pairs)) {
  ...
  for (i in seq_along(intervals)) {
    ...
    max_possible_rmse_value <- 7  # assuming max possible RMSE is 7 for each interval
...
Overall_max_rmse <- max(result_df$Max_RMSE)
# Max RMSE value
print(Overall_max_rmse)
}
{
...
# Similarity percentages between training datasets
print(Similarity_results_1)
}
# Take the norm values column of a test(validation) dataset and compare its similarity percentages with the training datasets
# We filter the result_df to check the similarity percentages of Breast cancer dataset (c)(validation dataset) with the training datasets by selecting the column Norm.B.c. 
...
# Similarity percentages of the selected validation dataset with the training datasets
print(Similarity_results_2)
  }
# Save the results to a CSV file
write.csv(Similarity_results_2, "D:/My PhD information folder first Paper/Data Analysis in R Modified/Similarity_Percentages2.csv", row.names = FALSE)

...
# script1.R (example file used for illustration: "V1_Breast_Cancer_Block_A_Section_1_filtered_feature_bc_matrix.h5")
...
# Use Table S1 from the paper to select the minimum and maximum features threshold to filter different datasets for which  required
...
# Second, Checked the similarity percentage of a validation or test dataset with the training datasets(norm values). For any new random dataset to check the similarity percentage with the training datasets, just add its norm value dataset with Table 3 and procede to next step
